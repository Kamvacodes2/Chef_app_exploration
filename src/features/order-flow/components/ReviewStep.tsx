    ...
    <div className="flex items-center gap-3 rounded-3xl bg-white/[0.04] p-3 ring-1 ring-white/10">
      {kind !== "dessert" ? (
        <div className={"relative shrink-0 overflow-hidden bg-[var(--color-oxblood)] " + imageSize}>
          <Image src={item.imageSrc} alt={item.imageAlt} fill sizes={kind === "main" ? "64px" : "48px"} className="object-cover" />
        </div>
      ) : null}
      ...
      <p className="shrink-0 text-sm font-semibold text-[var(--color-bone)]">
        {includedInPlan ? "Included" : priceCents === undefined ? "Pending" : formatZarCents(priceCents)}
      </p>
    </div>
  );
}
...
export function ReviewStep(): ReactElement {
  ...
  const address = [state.address.unit, state.address.street, state.address.area].filter(Boolean).join(", ");
  ...
  const plan = pricingQuote?.plan ?? (catalogPlan
    ? {
        id: catalogPlan.id,
  ...
  const favourite = state.favoriteMealId ? findItem(state.favoriteMealId)?.name ?? "Your selected favourite" : null;
    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <div className="grid w-full gap-4 lg:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-3">
          {state.main ? <SelectionRow item={state.main} kind="main" priceCents={priceFor(state.main)} includedInPlan={Boolean(pricingQuote?.plan)} /> : null}
          {state.sides.map((side) => (
            <SelectionRow key={side.id} item={side} kind="side" priceCents={priceFor(side)} includedInPlan={Boolean(pricingQuote?.plan)} />
          ))}
          {state.dessert ? <SelectionRow item={state.dessert} kind="dessert" priceCents={priceFor(state.dessert)} includedInPlan={Boolean(pricingQuote?.plan)} /> : null}

          {plan ? (
            <div className="flex flex-col gap-1 rounded-3xl bg-white/[0.04] p-4 ring-1 ring-white/10">
              <p className="text-xs font-bold uppercase tracking-wider text-[var(--color-bone)]/60">Package preferences</p>
              <p className="text-sm font-semibold text-[var(--color-bone)]">{plan.name} - {plan.sessions}</p>
              {plan.recurring ? (
          ...
          <div className="mt-2 flex flex-col gap-1 rounded-3xl bg-white/[0.04] p-4 ring-1 ring-white/10">
            ...
            {state.address.estate ? <p className="text-sm text-[var(--color-bone)]/80">{state.address.estate}</p> : null}
            {contactName || state.contact.phone ? (
              <p className="mt-1 text-sm text-[var(--color-bone)]/80">{[contactName, state.contact.phone].filter(Boolean).join(" | ")}</p>
            ) : null}
            {contactEmail ? <p className="text-sm text-[var(--color-bone)]/80">{contactEmail}</p> : null}
          </div>
        </div>

        <aside className="flex h-fit flex-col gap-4 rounded-3xl bg-[var(--color-bone)] p-5 text-[var(--color-oxblood)] lg:sticky lg:top-4">
          <h3 className="font-display text-2xl font-semibold">{isCustomRequest || isPlanRequest ? "What happens next" : plan ? "Your package" : "What happens next"}</h3>
          <div className="flex flex-col gap-2 border-b border-[var(--color-oxblood)]/15 pb-3">
            {plan ? <p className="text-sm font-semibold text-[var(--color-oxblood)]/78">{plan.name} - {plan.sessions}</p> : null}
            <div className="flex items-baseline justify-between gap-4">
              <span className="text-sm text-[var(--color-charcoal)]/75">
                {isCustomRequest ? "Custom quote" : isPlanRequest ? "Monthly plan" : isPricingLoading ? "Updating total" : plan ? "Session total" : "Order total"}
              </span>
              <span className="font-display text-2xl font-semibold">{isCustomRequest ? "To be confirmed" : totalCents === undefined ? "--" : formatZarCents(totalCents)}</span>
            </div>
            <div className="flex items-center justify-between gap-4 text-xs text-[var(--color-charcoal)]/65">
              ...
              <span>{isCustomRequest ? "Chefmate will confirm" : subtotalCents === undefined ? "--" : formatZarCents(subtotalCents)}</span>
            </div>
            {discountCents > 0 ? (
              <div className="flex items-center justify-between gap-4 text-xs text-[var(--color-charcoal)]/65">
                <span>Discount</span>
