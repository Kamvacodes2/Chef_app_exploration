...
import { buildPricingQuotePayload, fetchPricingQuote, type PricingQuote } from "../api/pricingQuoteClient";
import {
  INITIAL_ORDER_STATE,
  orderReducer,
  selectCanContinue,
...
function submissionMessage(error: unknown): string {
  if (error instanceof Error && error.message.includes("NEXT_PUBLIC_CHEFMATE_API_URL")) return error.message;
...
export function useOrderController(): OrderController {
  ...
  const canContinue = useMemo(
    () => (state.step === "address" && isSessionLoading ? false : selectCanContinue(state, usesAccountContact)),
  ...
  const confirm = useCallback(async () => {
    ...
    if (isPricingLoading || !pricingQuote) {
      setSubmissionError("We need to confirm the latest total before checkout. Please wait a moment and try again.");
      return;
    }
    ...
    try {
      ...
      const confirmation = await submitBookingRequestPayload(payload, { idempotencyKey: idempotencyKeyRef.current });
