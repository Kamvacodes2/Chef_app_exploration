...
function validatedGuestContact(contact: ContactDetails): ContactDetails {
  ...
  if (!/^\S+@\S+\.\S+$/.test(contact.email.trim())) throw new Error("A valid contact email is required.");
...
export async function submitBookingRequestPayload(
  ...
  const response = await fetchImpl(apiUrl(options.baseUrl ?? getChefmateApiUrl(), "/api/v1/booking-requests"), {
    method: "POST",
    credentials: "include",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(await readApiErrorMessage(response, "Chefmate booking request failed (" + response.status + ")"));
  }
