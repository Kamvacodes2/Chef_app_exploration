...
export async function fetchPricingQuote(
  ...
  options: { readonly signal?: AbortSignal; readonly baseUrl?: string; readonly fetchImpl?: typeof fetch } = {},
): Promise<PricingQuote> {
  const fetchImpl = options.fetchImpl ?? fetch;
  const response = await fetchImpl(apiUrl(options.baseUrl ?? getChefmateApiUrl(), "/api/v1/booking-requests/quote"), {
    method: "POST",
    credentials: "include",
