 ...
 */
const GIFT_CODES: Readonly<Record<string, { discountFraction: number; label: string }>> = Object.freeze({
  "CHILL10": Object.freeze({ discountFraction: 0.1, label: "10% off applied" }),
  "WINTER15": Object.freeze({ discountFraction: 0.15, label: "15% winter warmer discount applied" }),
...
export function validateGiftCode(raw: string): GiftCodeResult {
  ...
  return { valid: false, message: "That code doesn't look right — try CHILL10.", discountFraction: 0 };
}
