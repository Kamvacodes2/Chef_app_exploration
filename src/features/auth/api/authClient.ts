...
export async function signIn(input: SignInInput, options: AuthRequestOptions = {}): Promise<AuthenticatedUser> {
...
export async function getCurrentUser(options: AuthRequestOptions = {}): Promise<AuthenticatedUser | null> {
  const fetchImpl = options.fetchImpl ?? fetch;
  const response = await fetchWithTimeout(fetchImpl, apiUrl(options.baseUrl ?? getChefmateApiUrl(), "/api/v1/auth/me"), {
    method: "GET",
    credentials: "include",
  });
...
async function sendAuthRequest(
  ...
  const fetchImpl = options.fetchImpl ?? fetch;
  const response = await fetchWithTimeout(fetchImpl, apiUrl(options.baseUrl ?? getChefmateApiUrl(), path), {
    method: "POST",
    credentials: "include",
...
async function readErrorMessage(response: Response): Promise<string> {
  return readApiErrorMessage(response, "Chefmate could not complete this request (" + response.status + ").");
}
