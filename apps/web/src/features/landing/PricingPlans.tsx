    ...
    >
      <div className="mx-auto w-full max-w-[1200px] px-4 sm:px-6 lg:px-8">
        <div className="mx-auto mb-8 max-w-2xl text-center">
          <h2
            id="plans-heading"
            className="flex flex-wrap items-baseline justify-center gap-x-3 gap-y-1 font-display text-4xl leading-none text-[#611a1e] sm:text-5xl"
          >
          ...
          {CHEFMATE_PLANS.map((plan) => (
            <div key={plan.id} className="relative pt-6">
              ...
              >
                ...
                />
                <span
                  className={`absolute right-4 top-4 z-10 inline-flex min-h-7 items-center gap-1.5 rounded-full px-2.5 text-xs font-bold shadow-sm backdrop-blur-sm ${TIER_STYLES[plan.tier]}`}
                >
                ...
                >
                  ...
                  <span className="mt-3 block font-display text-2xl leading-none sm:text-[1.7rem]">
                    {plan.name}
                  </span>
                  <span className="mt-2 block text-base font-bold leading-6 sm:text-lg">
                    {plan.price}
                  </span>
                  <span className="mt-2 block text-sm leading-5 text-white/90">
                    {plan.description}
                  </span>
                </span>
              </a>
