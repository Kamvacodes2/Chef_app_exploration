...
describe("LandingPage", () => {
  ...
  it("places the final callout image before its copy on mobile only", () => {
    ...
    const copyPanel = screen.getByRole("heading", { name: "Give yourself the evening back." }).parentElement;
  ...
  it("guides a recurring package through days and a favourite meal before the booking flow", async () => {
    ...
    const finalCallout = screen.getByRole("heading", { name: "Give yourself the evening back." }).closest("section");
    ...
    expect(screen.getByAltText("Two people enjoying a freshly cooked meal together at home")).toHaveAttribute(
      "src",
      expect.stringContaining("chefmate_tonight.jpg"),
    );
    ...
    expect(screen.getByAltText("A family sharing a relaxed dinner together at home")).toHaveAttribute(
      "src",
      expect.stringContaining("chefmate_family.jpg"),
    );
    expect(screen.getByAltText("A family enjoying a generous home-cooked meal together")).toHaveAttribute(
      "src",
      expect.stringContaining("chefmate_full_house.jpg"),
    );
    expect(plansSection.compareDocumentPosition(finalCallout)).toBe(Node.DOCUMENT_POSITION_FOLLOWING);

    fireEvent.click(screen.getByRole("link", { name: /chefmate family/i }));

    expect(await screen.findByRole("heading", { name: "Which days suit your household?" })).toBeInTheDocument();
    ...
    expect(await screen.findByRole("heading", { name: "What would you like most often?" })).toBeInTheDocument();
  ...
  it("lets recurring package customers decide days and meals later", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "Find what you want to eat." })).toBeInTheDocument();
  ...
  it("returns to the neutral flow when browser history leaves a package link", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "What are you feeding?" })).toBeInTheDocument();
  ...
  it("keeps regular booking calls at the goal selection", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "What are you feeding?" })).toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Find what you want to eat." })).not.toBeInTheDocument();
    expect(screen.getByTestId("order-flow")).toHaveAttribute("data-step", "goal");
  });
});