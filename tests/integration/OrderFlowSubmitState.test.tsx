...
describe("OrderFlow submit state", () => {
  it("shows a compact visible error when submission fails", () => {
    controllerStore.current = controller({ submissionError: "We couldn't send this request. Please try again." });

    render(<OrderFlow />);

    expect(screen.getByRole("alert")).toHaveTextContent("We couldn't send this request. Please try again.");
  });
  ...
  it("uses a plan request action for recurring packages until session entitlements are activated", () => {
    controllerStore.current = controller({
      ...
      pricingQuote: {
        ...
        totalCents: 379900,
        items: [],
        plan: { id: "family", name: "chefmate family", sessions: "8 sessions", recurring: true, priceCents: 379900 },
      },
      isPricingLoading: false,
    });
