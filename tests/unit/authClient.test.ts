...
describe("authClient", () => {
  it("sends login requests with the browser session cookie enabled", async () => {
    ...
    await expect(signIn({ email: authenticatedUser.email, password: "A-strong-password-2026" }, { baseUrl: "http://api.test", fetchImpl }))
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        credentials: "include",
        body: JSON.stringify({ email: authenticatedUser.email, password: "A-strong-password-2026" }),
  ...
  it("creates a customer account and preserves useful API errors", async () => {
    const fetchImpl = vi.fn().mockResolvedValue({
      ok: false,
      status: 409,
      json: async () => ({ error: { code: "email_in_use", message: "An account with this email already exists" } }),
    }) as unknown as typeof fetch;

    await expect(createCustomerAccount({ displayName: "Sam", email: authenticatedUser.email, password: "A-strong-password-2026" }, { baseUrl: "http://api.test", fetchImpl }))
